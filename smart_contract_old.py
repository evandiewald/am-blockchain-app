import datetime
import os
from blockchain import Blockchain, new_block, get_timestamp
import pickle
import blockchain


class SmartContract(object):
    def __init__(self, blockchain_obj: Blockchain, user_role, contract_id=None):
        ...
        self.ledger_path = blockchain_obj.ledger_path
    ...
    def refresh(self, transaction_data, last_update_timestamp):
        updated, my_ledger_path = blockchain.ledger_updated(self.blockchain.cnx, self.blockchain.client,
                                                            self.blockchain.gpg)
        if not updated:
            self.blockchain = blockchain.download_ledger(self.blockchain.cnx, self.blockchain.client,
                                                         self.blockchain.gpg)
        ipfs_hash = blockchain.insert_transaction(self.blockchain.cnx, self.blockchain.client, self.blockchain.gpg,
                                                  self.contract_id, transaction_data,
        ...
        blockchain.update_world_state(self.blockchain.cnx, self.contract_id, last_update_timestamp,
                                      self.contract_header['current_state'])
        block_added, self.blockchain.ledger_path = new_block(self.blockchain, block, ipfs_hash)
        self.blockchain.chain = self.blockchain.load_blockchain(self.blockchain.ledger_path)
        print('Transaction block added to blockchain at ', self.blockchain.ledger_path, '\n', block_added)
        return block_added
    ...
    def deploy_contract(self, project_name, engineer_email, engineer_name, technician_email,
        ...
        blockchain.upload_contract_header(self.blockchain.cnx, self.contract_id, deployment_timestamp, project_name,
                                          ...
                                          self.contract_header['current_state'])
        block_added = self.refresh(transaction_data, last_update_timestamp)
        return block_added
